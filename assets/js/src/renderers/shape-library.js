;(function(undefined) {
  ...
  var drawCross = function(node,x,y,size,context) {
    var lineWeight = (node.cross && node.cross.lineWeight) || 3;
  ...
  var drawTriangle = function(node,x,y,size,context) {
    var pcount = (node.triangle && node.triangle.numPoints) || 3;
    var rotate = ((node.triangle && node.triangle.rotate) || 0)*Math.PI/180;
    var radius = size;
    context.moveTo(x+radius*Math.sin(rotate), y-radius*Math.cos(rotate)); // first point on outer radius, angle 'rotate'
    for(var i=1; i<pcount; i++) {
      context.lineTo(x+Math.sin(rotate+2*Math.PI*i/pcount)*radius, y-Math.cos(rotate+2*Math.PI*i/pcount)*radius);
  ...
  var drawTriangle180 = function(node,x,y,size,context) {
    var pcount = (node.triangle180 && node.triangle180.numPoints) || 3;
    var rotate = ((node.triangle180 && node.triangle180.rotate) || 180)*Math.PI/180;
    var radius = size;
    context.moveTo(x+radius*Math.sin(rotate), y-radius*Math.cos(rotate)); // first point on outer radius, angle 'rotate'
    for(var i=1; i<pcount; i++) {
      context.lineTo(x+Math.sin(rotate+2*Math.PI*i/pcount)*radius, y-Math.cos(rotate+2*Math.PI*i/pcount)*radius);
  ...
  register("triangle180",genericDrawShape(drawTriangle180),genericDrawBorder(drawTriangle180));
